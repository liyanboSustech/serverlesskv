...
# ABC是一个抽象基类，它的作用是定义一些抽象方法，然后由子类去实现这些方法。这样可以保证子类一定会实现这些方法，否则会报错。
class Element(ABC):
    name: Union[None, str]
    offset: int
...
class Parameter(Element):
    ...
    # 这个函数的作用是什么
    # 这个函数的作用是将xml文件中的parameter标签解析成一个Parameter对象
    # 这个函数接受两个参数，一个是spec，一个是lm
    # spec是一个lxml.etree.Element对象
    # lm是一个LanguageModel对象
...
# tokensequence这个类的作用是什么
# token_sequence这个类的作用是将文本转换成token序列
...
class Module(Element):
    ...
    def __init__(self,
        ...
        # 如果spec是字符串，那么就用lxml.etree.fromstring(spec)来解析
        # 解析完,spec就是一个lxml.etree.Element对象
        if type(spec) == str:
